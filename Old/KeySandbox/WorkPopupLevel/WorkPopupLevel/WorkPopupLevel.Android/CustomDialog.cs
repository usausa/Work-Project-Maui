...
namespace WorkPopupLevel.Droid
{
    public class CustomDialog : ICustomDialog
    {
        ...
        public async ValueTask<bool> Confirm(string title, string message, string ok, string cancel)
        {
            var dialog = new ConfirmDialog(activity);
            return await dialog.ShowAsync(title, message, ok, cancel);
        }

        public async ValueTask Information(string title, string message, string ok)
        {
            var dialog = new InformationDialog(activity);
            await dialog.ShowAsync(title, message, ok);
        }

        public async ValueTask<int> Select(int selected, string[] items)
        {
