...
- ポート: 8080 = Web / API(HTTP/1.1)、9090 = gRPC(HTTP/2 h2c、アプリケーション用。gRPC のサービスはこのポートに限定)、4317 = OTEL 用(HTTP/2 h2c、後で OTLP の受け口に使う。現状は未使用)。管理画面は `http://localhost:8080/`(認証なし)
- USB 接続の端末は `adb reverse tcp:8080 tcp:8080` と `adb reverse tcp:9090 tcp:9090` で端末側の `localhost` を PC へ転送する(LAN の端末は PC の IP で接続)
- 端末の設定は管理画面の `/qr` が出す QR を Main > Setting の「Scan configuration QR」で読み取る(`ApiEndPoint` / `GrpcEndPoint` の既定値はサーバー自身の URL、AI サービス / Ollama / SSH の値はサーバーの `Setting` テーブル = `/qr` で編集した値から)
