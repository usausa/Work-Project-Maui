...
Nalu Scaffold の機能のうち、`MainPage`(ヘッダ `Grid` + `AbsoluteLayout` コンテナ + フッタ `Grid`)に無い 4 点のうち残り 2 点(6-1 ステータスバーは実装済み = `Change_Summary.md` 区間10、6-4 予測型バックは現状維持で確定 = 同 付録D D25)。静的分析で結論を出さず、**実機(Pixel 9a / Android 16 / Release)で裏取りしてから実装する**。BACK / Activity 再生成の再現手順は `Change_Summary.md` 区間6 B-3、決定の経緯は同 付録D D23 / D24。
